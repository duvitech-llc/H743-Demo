...
static void USBD_CMPSIT_HISTODesc(USBD_HandleTypeDef *pdev, uint32_t pConf, __IO uint32_t *Sze, uint8_t speed);
#endif

#if USBD_CMPSIT_ACTIVATE_IMU == 1U
static void USBD_CMPSIT_IMUDesc(USBD_HandleTypeDef *pdev, uint32_t pConf, __IO uint32_t *Sze, uint8_t speed);
#endif /* USBD_CMPSIT_ACTIVATE_IMU == 1U */

#if USBD_CMPSIT_ACTIVATE_COMMS == 1U
static void USBD_CMPSIT_COMMSDesc(USBD_HandleTypeDef *pdev, uint32_t pConf, __IO uint32_t *Sze, uint8_t speed);
#endif /* ifUSBD_CMPSIT_ACTIVATE_COMMS == 1U */
...
uint8_t  USBD_CMPSIT_AddToConfDesc(USBD_HandleTypeDef *pdev)
{
  ...
  switch (pdev->tclasslist[pdev->classId].ClassType)
  {
...
#if USBD_CMPSIT_ACTIVATE_COMMS == 1
    case CLASS_TYPE_COMMS:
		/* Setup Max packet size for COMMS */
		pdev->tclasslist[pdev->classId].CurrPcktSze = (pdev->dev_speed == USBD_SPEED_HIGH) ? COMMS_HS_MAX_PACKET_SIZE : COMMS_FS_MAX_PACKET_SIZE;
		...
		pdev->tclasslist[pdev->classId].NumEps = 2U; /* EPx_IN */
		...
		/* Configure and Append the Descriptor */
		USBD_CMPSIT_COMMSDesc(pdev, (uint32_t)pCmpstFSConfDesc, &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);
#ifdef USE_USB_HS
		USBD_CMPSIT_COMMSDesc(pdev, (uint32_t)pCmpstHSConfDesc, &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
    break;
#endif
    ...
    case CLASS_TYPE_HISTO:
        ...
        /* Configure and Append the Descriptor */
        USBD_CMPSIT_HISTODesc(pdev, (uint32_t)pCmpstFSConfDesc, &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);
#ifdef USE_USB_HS
        USBD_CMPSIT_HISTODesc(pdev, (uint32_t)pCmpstHSConfDesc, &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
...
#if USBD_CMPSIT_ACTIVATE_COMMS == 1
/**
  * @brief  USBD_CMPSIT_COMMSDesc
  *         Configure and Append the COMMS Descriptor
  * @param  pdev: device instance
  * @param  pConf: Configuration descriptor pointer
  * @param  Sze: pointer to the current configuration descriptor size
  * @retval None
...
static void USBD_CMPSIT_COMMSDesc(USBD_HandleTypeDef *pdev, uint32_t pConf, __IO uint32_t *Sze, uint8_t speed)
{
	  ...
	  {
	    ep_size = COMMS_HS_MAX_PACKET_SIZE;
	  }
	  else
	  {
	    ep_size = COMMS_FS_MAX_PACKET_SIZE;
	  }
	  ...
	  __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add, /* EP address */
	                      USBD_EP_TYPE_BULK,                          /* EP type */
	                      ep_size,                                    /* Max packet size */
	                      (speed == USBD_SPEED_HIGH) ? 0U : 0U,       /* HS bulk interval = 0 */
	                      0U);                                        /* Refresh/interval */

	  /* Append Endpoint descriptor (7 bytes) */
	  __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[1].add, /* EP address */
	                      USBD_EP_TYPE_BULK,                          /* EP type */
	                      ep_size,                                    /* Max packet size */
	                      (speed == USBD_SPEED_HIGH) ? 0U : 0U,       /* HS bulk interval = 0 */
	                      0U);                                        /* Refresh/interval */
...
/**
  * @brief  USBD_CMPSIT_HISTODesc
  *         Configure and Append the HISTO Descriptor
  * @param  pdev: device instance
  * @param  pConf: Configuration descriptor pointer
  * @param  Sze: pointer to the current configuration descriptor size
  * @retval None
  */
static void USBD_CMPSIT_HISTODesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
        __IO uint32_t *Sze, uint8_t speed)
{
	  USBD_IfDescTypeDef *pIfDesc;
	  USBD_EpDescTypeDef *pEpDesc;
...
/**
  * @brief  USBD_CMPSIT_IMUDesc
  *         Configure and Append the IMU Descriptor
  * @param  pdev: device instance
  * @param  pConf: Configuration descriptor pointer
  * @param  Sze: pointer to the current configuration descriptor size
  * @retval None
  */
static void USBD_CMPSIT_IMUDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                               __IO uint32_t *Sze, uint8_t speed)
{
  USBD_IfDescTypeDef *pIfDesc;
  USBD_EpDescTypeDef *pEpDesc;
  ...
  if (speed == USBD_SPEED_HIGH)
  {
    ep_size = IMU_HS_MAX_PACKET_SIZE;
  }
  else
  {
